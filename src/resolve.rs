...
pub fn collect_sources(imports: &[String], reexports: &[ReExport]) -> Vec<String> {
...
pub fn load_path_config(start_dir: &Path) -> Option<PathConfig> {
    ...
    let val: serde_json::Value = match serde_json::from_str(&stripped) {
        Ok(v) => v,
        Err(e) => {
            eprintln!("xray: warning: failed to parse {}: {e}", tsconfig_path.display());
            return None;
        }
    };
...
fn try_extensions(base: &Path) -> Option<PathBuf> {
    ...
    if let Some(ext) = base.extension().and_then(|e| e.to_str()) {
        if EXTENSIONS.contains(&ext) && base.is_file() {
...
mod tests {
  ...
  "compilerOptions": {
    ...
    fn resolve_import_parent_directory() {
        ...
        let result = resolve_import("../utils", &entry, None)
